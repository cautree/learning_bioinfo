...
df_rep1 = df_res %>% 
  ...
  dplyr::mutate( real_label = ifelse( grepl("G", well),PRECISION, "" ))



df_rep1 %>% 
  ggplot(aes( depth, PRECISION, colour = method )) +
  geom_point() +
  geom_line() +
  ylim(0.7,1) + 
  geom_text(aes(y = PRECISION, label= real_label), hjust= 0.5, vjust = -2, size= 2.0, color= "black") +
  xlab("x_depth")
...
df_rep2 = df_res %>% 
  ...
  dplyr::filter( depth !=2 ) %>% 
  dplyr::mutate( real_label = ifelse( grepl("Manual", method),PRECISION, "" )) %>% 
...
df_rep2 %>% 
  ggplot(aes( depth_group, PRECISION, colour = method )) +
  geom_point() +
  geom_line() +
...
ggsave("figures/PRECISION.jpg", width =15, height=6)
```
