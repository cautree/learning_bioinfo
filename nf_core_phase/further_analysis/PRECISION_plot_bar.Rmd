...
df_rep2 = df_res %>% 
  ...
  dplyr::mutate( real_label = PRECISION ) %>% 
...
df_rep2 %>% 
  ggplot(aes( depth_group, PRECISION, fill = method )) +
  geom_bar(stat = "identity", position = "dodge") +
  facet_grid( REP_group ~ input) +
#  geom_text(aes(y = PRECISION, label= real_label), hjust= 0.5, vjust = -2, size= 1.5, color= "black") +
  xlab("x_depth") +
  scale_x_continuous(breaks=seq(1,6,1),labels=my_labels)  +
  scale_fill_manual(values=cols)


ggsave("figures/PRECISION_bar.jpg", width =25, height=12)
