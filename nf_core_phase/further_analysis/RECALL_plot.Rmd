...
df_rep1 = df_res %>% 
  ...
  dplyr::mutate( real_label = ifelse( grepl("G", well),RECALL, "" ))



df_rep1 %>% 
  ggplot(aes( depth, RECALL, colour = method )) +
  geom_point() +
  geom_line() +
  ylim(0.7,1) + 
  geom_text(aes(y = RECALL, label= real_label), hjust= 0.5, vjust = -2, size= 2.0, color= "black") +
  xlab("x_depth")
...
df_rep2 %>% 
  ggplot(aes( depth_group, RECALL, colour = method )) +
  geom_point() +
  geom_line() +
...
ggsave("figures/RECALL.jpg", width =15, height=6)
```
