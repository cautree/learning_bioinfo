...
read_file = function (x){
  ...
  df = df %>% 
    ...
    dplyr::mutate( ACCURACY = 100*(TP+TN)/929834 ) %>% 
    dplyr::select( sample, depth, type,  everything()) %>% 
    dplyr::mutate( ACCURACY = round(ACCURACY,2)) %>% 
    dplyr::mutate( F1 = 2*TP/(2*TP+FP+FN))
  return(df)
...
readr::write_csv( df_res, "reports/GATK_concordance_with_F1.csv")
...
df_rep2 = df_res %>% 
 ...
 # dplyr::mutate( real_label = ifelse( grepl("Manual", method),ACCURACY, "" )) %>% 
  dplyr::mutate( real_label = ACCURACY ) %>% 
...
df_rep2 %>% 
  ggplot(aes( depth_group, ACCURACY, fill = method )) +
  geom_bar(stat = "identity", position = "dodge") +
  facet_grid( REP_group ~ input) +
#  geom_text(aes(y = ACCURACY, label= real_label), hjust= 0.5, vjust = -2, size= 2.0, color= "black") +
  xlab("x_depth") +
  scale_x_continuous(breaks=seq(1,6,1),labels=my_labels) +
  scale_fill_manual(values=cols)

ggsave("figures/ACCURACY.jpg", width =15, height=12)
...
df_rep2 = df_res %>% 
  ...
  dplyr::filter( depth !=2 ) %>% 
 # dplyr::mutate( real_label = ifelse( grepl("Manual", method),ACCURACY, "" )) %>% 
  dplyr::mutate( real_label = F1 ) %>% 
  dplyr::mutate( REP_group = ifelse( grepl("(01|03|05|07|09|11)", well), "Rep1", "Rep2")) %>% 
  dplyr::mutate( input = factor(input, levels = c("5ng", "10ng", "20ng", "25ng", "40ng","50ng"), ordered = T ))


df_rep2 %>% 
  ggplot(aes( depth_group, F1, fill = method )) +
  geom_bar(stat = "identity", position = "dodge") +
  facet_grid( REP_group ~ input) +
  xlab("x_depth") +
  scale_x_continuous(breaks=seq(1,6,1),labels=my_labels) +
  scale_fill_manual(values=cols) +
  coord_cartesian(ylim=c(0.8,0.91)) +
  scale_y_continuous( breaks = seq(0.8, 0.91, by = 0.01))


ggsave("figures/ACCURACY_F1.jpg", width =15, height=12)
