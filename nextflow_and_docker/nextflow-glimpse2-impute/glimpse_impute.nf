params.run = "20241210_Admera_health"
params.analysis = "Human_1x"


process glimpse_impute {

  //publishDir path: "glimpse_impute", mode: 'copy'
  //publishDir path: "s3://seqwell-analysis/$params.run/$params.analysis/glimpse_impute"
...
process ligate {
  ...
  publishDir path: "s3://seqwell-analysis/$params.run/$params.analysis/glimpse_impute/ligated_vcf"
  ...
  output:
  tuple val(pair_id), path("${pair_id}*ligated.vcf.gz*")
...
workflow {
 ...
 bam_index_file_ch = Channel
                     .fromPath("s3://seqwell-analysis/" + params.run + "/" + params.analysis + "/simulated_bam/*.bam") 
                     .map { bam_file -> 
